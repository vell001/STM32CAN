...
#define DEBUG(x) Serial1.println(x); // Debug disabled
...
public:
    ...
    /* user interface */
    int begin( void ) { return begin( CAN_DEFAULT_BAUD, false, true ); }; // begin with default baudrate
    int begin( int baudrate ) { return begin( baudrate, false, true ); }; // begin with user defined baudrate
    int begin( int baudrate, bool loopBack ) { return begin( baudrate, loopBack, true ); }; // begin with option to enable loopback
    int begin( int baudrate, bool loopBack, bool UseAltPins); // begin with user defined baudrate and option for using loopback and alternative pins
...
private:
    ...
    int      init( CAN_HandleTypeDef* CanHandle, int baudrate, bool UseAltPins, bool loopBack );
